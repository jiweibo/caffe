...
void SmoothL1LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < count; ++i) {
    diff_.mutable_cpu_data()[i] =
        bottom[0]->cpu_data()[i] - bottom[1]->cpu_data()[i];
    if (has_weights_) {
      diff_.mutable_cpu_data()[i] =
          bottom[2]->cpu_data()[i] * diff_.cpu_data()[i];
    }
    if (abs(diff_.cpu_data()[i]) < 1.0 / sigma2_) {
      errors_.mutable_cpu_data()[i] =
          0.5 * diff_.cpu_data()[i] * diff_.cpu_data()[i] * sigma2_;
    } else {
      errors_.mutable_cpu_data()[i] = abs(diff_.cpu_data()[i]) - 0.5 / sigma2_;
    }

    if (has_weights_) {
      errors_.mutable_cpu_data()[i] =
          bottom[3]->cpu_data()[i] * errors_.cpu_data()[i];
    }
  }
...
void SmoothL1LossLayer<Dtype>::Backward_cpu(
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      caffe_cpu_axpby(count, alpha, diff_.cpu_data(), Dtype(0),
                      bottom[i]->mutable_cpu_diff());
      if (has_weights_) {
        // Scale by "inside" weight
        caffe_mul(count, bottom[2]->cpu_data(), bottom[i]->cpu_diff(),
                  bottom[i]->mutable_cpu_diff());
        // Scale by "outside" weight
        caffe_mul(count, bottom[3]->cpu_data(), bottom[i]->cpu_diff(),
                  bottom[i]->mutable_cpu_diff());
      }
    }
  }
