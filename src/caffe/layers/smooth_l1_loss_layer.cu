...
void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (has_weights_) {
    ...
    caffe_gpu_mul(
        count, bottom[3]->gpu_data(), errors_.gpu_data(),
...
void SmoothL1LossLayer<Dtype>::Backward_gpu(
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      caffe_gpu_axpby(count, alpha, diff_.gpu_data(), Dtype(0),
                      bottom[i]->mutable_gpu_diff());
      if (has_weights_) {
        // Scale by "inside" weight
        caffe_gpu_mul(count, bottom[2]->gpu_data(), bottom[i]->gpu_diff(),
                      bottom[i]->mutable_gpu_diff());
        // Scale by "outside" weight
        caffe_gpu_mul(count, bottom[3]->gpu_data(), bottom[i]->gpu_diff(),
                      bottom[i]->mutable_gpu_diff());
      }
    }
  }
